...
def scaled_dot_product_attention(Q, K, V):
    ...
    # 1. Tinh scores = Q @ K^T / sqrt(d_k)
    # MatMul giua Q va K.transpose de tim moi tuong quan
    scores = torch.matmul(Q, K.transpose(-2, -1)) / math.sqrt(d_k)

    # 2. Ap dung softmax tren chieu cuoi (dim=-1)
    # De cac attention weights cua moi tu tong bang 1.0
    weights = F.softmax(scores, dim=-1)

    # 3. Tinh output = weights @ V
    # Tong hop thong tin tu cac gia tri V dua tren trong so attention
